...
def _merge_index(
    ...
    if batch_id is not None and tmp_output_folder is not None:
        _save_small_index(index=merged_index, batch_id=batch_id, small_indices_folder=tmp_output_folder)
    return merged_index
...
def _merge_to_n_indices(spark_session, n: int, src_folder: str):
    ...
    rdd.foreach(
        lambda x: _merge_index(
            small_indices_folder=src_folder, batch_id=x[0], start=x[1], end=x[2], tmp_output_folder=dst_folder
        )  # type: ignore
    )
    fs = _get_file_system(src_folder)
...
def run(
    ...
    with Timeit("-> Merging indices", indent=2):
        ...
        if nb_indices_to_keep == 1:
            merged_index = _merge_index(next_stage_folder)
            fs.rm(temporary_indices_folder, recursive=True)
